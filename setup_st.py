...
# 7. This function is designed to capture the user's preferences for how the chatbot should respond.
def get_user_config():
    ...
    # Display button choices in the sidebar of the app for the user to pick their desired model
    st.sidebar.markdown("<b style='color: darkgreen;'>Choose a GPT model:</b>", unsafe_allow_html=True)
    model_name = st.sidebar.radio("", list(model_options.keys()), index=0, label_visibility="collapsed")

    # Display a slider option for the user to choose 'temperature' or randomness of the chatbot responses.
    ...
    # Display an input text box to capture user's OpenAI API key
...
# 8. Function to handle chatbot responses
...
# 9. Main function to handle interaction with the chatbot
def main():
    set_design()
